...
> 配套：`README.md`（入口）、`docs/adr/`（设计决策）、`docs/orchestration.md`（编排细节）、
> `docs/execution-trace.md`（可观测）、`docs/failure-taxonomy.md`（失败分类）。
...
```text
                        ...
                        │  (唯一运行时循环) │  + workflow/insurance-analysis.yaml（声明式单一真源）
                        └────────┬─────────┘
                                 │
                        ┌────────▼─────────┐
                        │     CaseState    │  state/case_state.py   ← 跨 Skill 唯一事实源
                        │ artifacts/tasks/ │  state/transitions.py  ← 单调性 / 前置条件 / 冻结
                        │ registry/evals/  │  workflow/artifact_registry.py ← 血缘 + 指纹
                        └────────┬─────────┘
                                 │
                        ┌────────▼─────────┐
...
| `RAG` | 共享 Evidence Provider（`rag/` 存储 + `evidence/loop.py` 受控回环） | `knowledge-search`（`services:` 中的非线性 Provider，不是 stage） |
...
| 组件 | 文件 | 职责 |
|---|---|---|
| Orchestrator | `workflow/orchestrator.py` | `next_runnable → run → eval → PASS/FAIL → repair → checkpoint` 主循环；闸门策略 `stop` |
| Workflow 声明 | `workflow/insurance-analysis.yaml` | stage 顺序 / artifact 生产消费 / executor / gate 的**单一真源** |
| CaseState | `state/case_state.py` | `artifacts / artifact_registry / tasks / evaluations / checkpoints / services / events / trace` |
| Transitions | `state/transitions.py` | 三不变量：**单调性**（NON_MONOTONIC）/ **前置条件**（MISSING_INPUT_ARTIFACT + INPUT_NOT_RELEASED）/ **冻结**（ARTIFACT_MUTATION，sha256） |
| Artifact Registry | `workflow/artifact_registry.py` | ART-ID / lineage / fingerprint / evidence_refs；只存元数据，不复制内容 |
| Eval Engine | `workflow/eval_engine.py` | 6 类确定性检查：schema / required_fields / contamination / provenance / cross_artifact / invariant（+ required_non_empty） |
| Repair | `workflow/repair.py` | 失败检查 → 动作映射（RERUN_FROM_UPSTREAM / DROP_INVALID_PRODUCTS）；只改**输入**，不改已冻结 artifact |
| Checkpoint | `workflow/checkpoint.py` | save / load / validate（5 项）→ 损坏即 `CHECKPOINT_INVALID`，绝不静默续跑 |
| Trace | `workflow/trace.py` | 结构化 Execution Trace（13 事件类型 + `duration_ms`），外置 `<case_dir>/trace.jsonl` |
| Observability | `workflow/observability.py` | per-skill latency / 调用计数 / repair 次数 / knowledge-search 次数，回答「一个 Case 跑完多少次 Skill 调用」 |
