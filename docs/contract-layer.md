...
> Phase 0 基线见 `docs/architecture-v2-audit.md`（**本文件不修改审计基线**）。
...
> **输出形态与 `recommendation_adapter` 均未变。** 详见 `docs/product-recommendation-v2.md`。
...
**Phase 5 / Phase 6 是首次修改既有 Skill**（均由用户 spec 明确授权）：Phase 5 改 `recommendation` 3 个文件（输入契约 + 三态约束护栏），Phase 6 改 `report-generation` 输入契约、适配器、引擎与规则（新增 canonical 章节 + 渲染修复）。两者均**未改写既有评估语义**——各自 legacy 数据集全部保持通过。详见 `docs/product-recommendation-v2.md` / `docs/report-generation-v2.md`。

### 5.4 Evidence Provider（Phase 4）

Knowledge Search 已固化为**共享 Evidence Provider**，原子操作是 `Skill → Evidence Request → Knowledge Search → Skill`，而非 workflow 里写死的一步。详见 `docs/evidence-provider.md`。

- `evidence/request.py`：请求侧，把任意 Skill 的需要转成规范 `KnowledgeQuery`。**查询由 `(domain, purpose)` 模板生成，不接受调用方自由文本**（防注入 + 防幻觉 + 可测）。
- `evidence/provider.py`：供给侧，`KnowledgeQuery → knowledge-search 引擎 → 规范 KnowledgeEvidence`，双向契约校验。
- `evidence/loop.py`：受控回环，返回 `source_unchanged` 证明**只读消费**。
...
- **不重造检索**：`evidence/provider.py` 加载并调用 knowledge-search 的既有入口；`knowledge-search` 0 文件被修改。
...
- **改动范围**：仅 `recommendation` 的 3 个文件（+52 / -8），评估逻辑未重写。
  详见 `docs/product-recommendation-v2.md`。
...
- **改动范围**：`report-generation` 的 input/output schema、适配器、引擎、规则、SKILL.md/CONTRACT.md，
  ...
  详见 `docs/report-generation-v2.md`。
...
- **State（`state/`）**：`CaseState` 是一个客户的**唯一事实源**；三不变量可机检——
...
- **Workflow（`workflow/insurance-analysis.yaml`）**：**声明式、单一真源**。7 个线性 stage
...
1. **`domain/insurance` Domain Pack**（pack / playbook / references / overlays）—— 按 Phase 1 评审
   **暂缓**至 8 Skill 跑稳之后；现条件已满足，于 **Phase 8** 落地（详见 `docs/domain-pack.md`）。
