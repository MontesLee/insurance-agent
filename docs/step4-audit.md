...
> 配套文档：`docs/step3-assembly.md`（Step 3 组装）、`docs/orchestration.md`（P7 架构）、`docs/architecture-v2.md`。
...
- **测试/演示驱动**：
  ...
  - `scripts/run-regression.ps1`（PowerShell 回归壳，本机 bash 不可用；另有 `tmp/run_regression.py`）。
...
| P6 Failure Taxonomy | 无文档化分类 | `docs/failure-taxonomy.md` F1–F12 + 检测/修复矩阵 | — |
