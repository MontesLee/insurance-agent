...
```
                 ...
                 │          Orchestrator         │
                 └───────────────┬───────────────┘
                                 │  (runtime/insurance-analysis.yaml + runtime/state/CaseState)
                                 ▼
...
- **Phase 4 已落地**为共享层 `evidence/`（`request.py` / `provider.py` / `loop.py`）；查询由 `(domain, purpose)` 模板生成，回环只读（`source_unchanged`），`knowledge-search` Skill 本身 0 改动。详见 `docs/architecture/evidence-provider.md`。
...
详见 `docs/dev-notes/step2-evidence-recommendation.md`。
...
| V2.1 | `domain/insurance/`（pack / playbook / references / overlays） | **已完成**（Phase 8，详见 docs/architecture/domain-pack.md） |
| V2.2 | `catalog/`（Demo Product Catalog）+ `product-candidate-provider` Skill | **已完成**（Step 2，详见 docs/dev-notes/step2-evidence-recommendation.md） |
...
| Phase | 范围 | 状态 |
|---|---|---|
| 0 | Architecture Audit（只读） | 完成（docs/dev-notes/architecture-v2-audit.md） |
| 1 | Canonical Contract Layer（9 契约 + 6 adapter + 9 契约测试） | 完成（docs/architecture/contract-layer.md）；已 STOP |
...
| 5 | product-recommendation 演化输入契约（根治 P1） | **完成**（docs/architecture/product-recommendation-v2.md）；已 STOP |
| 6 | report-generation 输入扩展至 8 Artifact | **完成**（docs/architecture/report-generation-v2.md）；已 STOP |
| 7 | case-state（`state/`）+ orchestrator（`workflow/`）+ E2E | **完成**（docs/architecture/orchestration.md）；已 STOP |
| 8 | `domain/insurance` Domain Pack（pack / playbook / references / overlays） | **完成**（docs/architecture/domain-pack.md）；已 STOP |
...
> 详见 `docs/architecture/domain-pack.md`。
