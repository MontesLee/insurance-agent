...
代码：`runtime/trace.py`（记录器）· `runtime/orchestrator.py`（发射点）· `runtime/checkpoint.py`
...
| `TASK_CREATED` | `runtime/tasks.py::init_tasks()` 每个 stage 建任务 |
...
> case 目录与 `case_state.json` 同级（`runtime/state/store.py::case_dir`），便于打包一个 case 的完整证据。
