...
|---|---|---|
| 行为契约 | Prompt/文档/代码三处写规则，久了漂移 | [CONTRACT.md](file:///D:/Workspace/insurance-agent/02-requirement-analysis/CONTRACT.md#L60-L170) + [input.schema.json](file:///D:/Workspace/insurance-agent/02-requirement-analysis/schemas/input.schema.json) + [output.schema.json](file:///D:/Workspace/insurance-agent/02-requirement-analysis/schemas/output.schema.json) 三者合一，所有脚本以 Schema 为准 |
| 信息充分性 | 「觉得够了就分析」靠体感 | [information-sufficiency.rules.json](file:///D:/Workspace/insurance-agent/02-requirement-analysis/config/information-sufficiency.rules.json#L14-L120) 定义 5 Scope 的 required/recommended/optional + weighted scoring + blocking fields + conflict fields，结果可解释、可回归、可逐 Scope 审计 |
| 问题生成 | 每次问一堆、重复问、机械问 | [question-generation.rules.json](file:///D:/Workspace/insurance-agent/02-requirement-analysis/config/question-generation.rules.json) 定义 priority formula（importance/impact/gap_type/difficulty/repeat_penalty），每轮最多 ≤3 条，重复字段自动跳过 |
...
2. **找输入**：优先读 `01-client-intake/clients/<客户>/CLIENT_PROFILE.md`（要求 Client Intake 已 `intake_complete=true`）；若你直接给了结构化 JSON 则直接用
3. **Adapter**：把 CLIENT_PROFILE.md 的事实转成符合 [input.schema.json](file:///D:/Workspace/insurance-agent/02-requirement-analysis/schemas/input.schema.json) 的 Input JSON——已知事实标 `KNOWN`，没说清的标 `ESTIMATED/UNKNOWN`，**绝不虚构**
...
```
你：张三的 intake 刚做完，客户目录是 01-client-intake/clients/zhangsan，帮我分析保障需求
...
Client Intake 已完成（CLIENT_PROFILE.md 存在 + intake_complete=true）
        │
        ▼
【Step 0 Adapter】把 CLIENT_PROFILE.md → requirement-analysis 的 Input JSON（见 §2.2）
        │
        ▼
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -InputJsonPath  .\02-requirement-analysis\tests\sufficiency\complete_info.input.json `
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -InputJsonPath  .\02-requirement-analysis\tests\questioning\priority_selection.input.json `
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -InputJsonPath  .\02-requirement-analysis\tests\analysis\life_complete.input.json `
...
powershell -NoProfile -ExecutionPolicy Bypass `
  ...
  -OutputJsonPath .\02-requirement-analysis\tests\dataset\dataset-report.json
...
Input Schema 完整定义在 [input.schema.json](file:///D:/Workspace/insurance-agent/02-requirement-analysis/schemas/input.schema.json)。生产环境调用前**先用 Phase 1 Schema Test 单测跑一下输入文件是否合法**，否则分析脚本会直接抛强错误：
...
### 3.3 统一运行时（PR-P01 生产加固核心）—— [requirement-analysis-runtime.ps1](file:///D:/Workspace/insurance-agent/scripts/lib/requirement-analysis-runtime.ps1#L1-L91)
...
#### 3.4.1 信息充分性规则 —— [information-sufficiency.rules.json](file:///D:/Workspace/insurance-agent/02-requirement-analysis/config/information-sufficiency.rules.json#L14-L120)
...
#### 3.4.2 追问优先级规则 —— [question-generation.rules.json](file:///D:/Workspace/insurance-agent/02-requirement-analysis/config/question-generation.rules.json)
...
三类场景详见 [DATASET.md](file:///D:/Workspace/insurance-agent/02-requirement-analysis/DATASET.md)：
...
1. **零修改原则**：`requirement-analysis` 绝对不能改 `.trae/skills/client-intake/**`、`01-client-intake/**`、`test-cases/client-intake/**`、`scripts/run-regression.ps1`。发现必须改这些地方才能跑通时，**停下并报告**，不要私自改 Client Intake 的 Prompt/Schema/测试。
2. **字段不匹配走 Adapter**：Client Intake 的 Confirmed Facts 是 Markdown 表格（年龄/收入/支出/家庭结构等），Requirement Analysis 的 Input 是结构化 JSON（facts[].field + facts[].value_status）。两者不一致时走 Adapter（在 `requirement-analysis` 自己的代码里做字段映射），而不是修改 Client Intake 的输出格式。
