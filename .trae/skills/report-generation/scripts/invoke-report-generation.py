"""CLI / importable entry point for Skill 6 Report Generation.

Usage:
    python invoke-report-generation.py --input input.json [--rules path] [--no-validate]
    echo '<json>' | python invoke-report-generation.py --stdin

Runs the deterministic engine, validates the output, and prints the structured
result (incl. rendered_report) to stdout. Exit code 0 = produced + valid; non-zero otherwise.
"""
...
def run(input_dict, rules=None, do_validate=True):
    out = generate_report(input_dict, rules)
...
def main():
    ...
    ap.add_argument("--input", help="path to report-generation input JSON")
    ap.add_argument("--stdin", action="store_true", help="read input JSON from stdin")
    ap.add_argument("--rules", default=None, help="path to report.rules.json")
