"""CLI / importable entry point for Skill 05 Solution (strategy layer).

Usage:
    python invoke-solution.py --input input.json [--rules path] [--no-validate]
    echo '<json>' | python invoke-solution.py --stdin

Input JSON (composite of Canonical artifacts, each an envelope or raw dict):
    {
      "coverage_gap_analysis": <CoverageGapAnalysis>,   # authoritative driver
      "requirement_analysis":  <RequirementAnalysis>,   # constraints
      "risk_assessment":       <RiskAssessment>         # context (referenced only)
    }

Runs the deterministic engine, wraps the strict SolutionPlan payload into the Canonical
...
def build_artifact(input_dict: dict, rules: Optional[dict] = None) -> dict:
    payload = analyze(
        input_dict.get("coverage_gap_analysis"),
...
def validate_output(artifact: dict) -> tuple:
...
def main():
    ...
    ap.add_argument("--rules", default=None, help="path to solution-mapping.rules.json")
