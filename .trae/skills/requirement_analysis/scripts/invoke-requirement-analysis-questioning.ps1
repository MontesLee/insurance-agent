...
$BaseDir = $Root
$QuestionRulesPath = Join-Path $BaseDir "resources\config\question-generation.rules.json"
...
# Field -> value_status map from facts. A field whose fact is explicitly UNKNOWN
# is not a real answer, so it must stay askable (see skip logic below). This
# prevents blocking questions from being silently dropped when an UNKNOWN field
# leaks into answered_fields (e.g. a client answered "I don't know" and the
# context updater stored it as answered+UNKNOWN).
$statusMap = @{}
foreach ($f in (To-JsonArray -Value $input.client_profile.facts)) {
    if (-not [string]::IsNullOrWhiteSpace($f.field)) {
        $statusMap[$f.field] = $f.value_status
    }
}
...
foreach ($gap in (To-JsonArray -Value $result.information_gaps)) {
    if ($answeredFields -contains $gap.field) {
        # An UNKNOWN fact is not a real answer — keep the field askable so a
        # blocking question is never silently suppressed. Fields with any other
        # status (KNOWN/ESTIMATED/ASSUMED) are genuinely answered and skipped.
        $vs = $statusMap[$gap.field]
        if ($vs -ne "UNKNOWN") {
            continue
        }
    }
